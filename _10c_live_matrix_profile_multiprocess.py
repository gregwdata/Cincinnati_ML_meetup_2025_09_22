...
# ----------------------------
print(os.getenv('PHYPHOX_ADDRESS'))
DEFAULT_PHONE_URL = 'http://' + os.getenv('PHYPHOX_ADDRESS')
...
def load_ref_sequences(reference_file: str) -> tuple[list[np.ndarray], list[str]]:
    ...
    acc = np.asarray(ref_data[0]["data"]["ts"]["acc"], dtype=float)
    acc_time = np.asarray(ref_data[0]["data"]["ts"]["acc_time"], dtype=float)
...
class DTWWorker:
    def __init__(self, ref_sequences: list[np.ndarray], ref_labels: list[str], settings: DTWSettings):
    ...
    def _compute(self):
        ...
        if (self._last_end_time is not None) and (self._last_end_time in times_np):
            ...
            new_data_mask = np.where(times_np > self._last_end_time)
            series_np = series_np[new_data_mask]
            times_np = times_np[new_data_mask]
            if len(series_np) < self.s.min_match_len:
                # skip evaluating if the number of points since last step is lower than min length
                return None

        best_label, best_start, best_end, best_dist = None, None, None, math.inf
        for q, label in zip(self.ref_sequences, self.ref_labels):
            sa = SubsequenceAlignment(q, series_np, use_c=True)
            bm = next(
                sa.best_matches_fast(minlength=self.s.min_match_len),
                None,
            )
            if bm is None:
                continue
            if bm.value < best_dist:
                s, e = bm.segment
                best_label, best_start, best_end, best_dist = label, int(s), int(e), float(bm.value)

        if best_label is None:
            return None
        if best_dist <= self.s.threshold:
    ...
    def run(self, in_q, out_q, stop_evt, poll_timeout=0.05):
        ...
        while not stop_evt.is_set():
            ...
            # Block briefly for the first item (so we don't busy-wait when idle)
            try:
                ...
                # # nothing arrived; you can still try a compute pass if you want
...
def run_ui(settings: Settings, ref_sequences: list[np.ndarray], ref_labels: list[str]):
    ...
    worker = DTWWorker(ref_sequences, ref_labels, DTWSettings(settings.min_match_len, settings.dtw_threshold, settings.dtw_buffer_size))
    proc = ctx.Process(target=worker.run, args=(dtw_in_q, dtw_out_q, stop_evt), daemon=True)
    proc.start()
...
def main():
    ...
    run_ui(settings, ref_sequences, ref_labels)
