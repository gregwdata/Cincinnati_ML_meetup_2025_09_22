...
def plot_data():
    ...
    def update_data():
        while not data_queue.empty():
            ...
            for i,subplot in enumerate(figure):
                #latest = x_data[i][-1]
                #lower_idx = np.max([0,np.where(np.array(x_data[i])>=(latest - PLOT_WINDOW))[0].min()])
                #new_data_length = len(x_data[i]) - lower_idx
                ...
                #new_data = np.column_stack((x_data[i][lower_idx:], y_data[i][lower_idx:], np.zeros((len(x_data[i])-lower_idx,1))))
            ...
            if len(fps_times) > 1:
                ...
                fps_text.offset = np.array([times[-1],0,0])
